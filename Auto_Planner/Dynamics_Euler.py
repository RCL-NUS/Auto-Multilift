...
class multilift_model:
    def __init__(self, sysm_para, dt_sample):
        ...
        self.el     = SX.sym('el',3,1) # Euler angle
        self.wl     = SX.sym('wl',3,1) # angular velocity in {Bl}
        self.xl     = vertcat(self.pl, self.vl, self.el, self.wl) # 12-by-1 vector
    ...
    def payload_dyn(self, xl, ul):
        ...
        # Quaternion
        el      = vertcat(xl[6,0], xl[7,0], xl[8,0])
        # Angular velocity in {Bl}
        wl      = vertcat(xl[9,0], xl[10,0], xl[11,0])
        ...
        # Kinematics
        dpl     = Rl@vl # position kinematics
        S_wl    = self.skew_sym(wl)
        KE      = vertcat(
                    horzcat(1,sin(el[0,0])*tan(el[1,0]),cos(el[0,0])*tan(el[1,0])),
                    horzcat(0,cos(el[0,0]),-sin(el[0,0])),
                    horzcat(0,sin(el[0,0])/cos(el[1,0]),cos(el[0,0])/cos(el[1,0]))
                )
        deuler  = KE@wl
        ...
        # Model
        model_l = vertcat(dpl, dvl, deuler, dwl)
    ...
    def minisnap_load_circle(self,Coeffx,Coeffy,Coeffz,time):
        ...
        ref_E = np.array([[0,0,0]]).T
        ref_w = np.zeros((3,1))
        ref_xl= np.reshape(np.vstack((ref_p,ref_v,ref_E,ref_w)),self.nxl)
