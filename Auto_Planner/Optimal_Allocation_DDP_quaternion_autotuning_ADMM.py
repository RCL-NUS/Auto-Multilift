...
class MPC_Planner:
    ...
    def SetLearnablePara(self):
        self.n_wsl    = self.n_xl # dimension of the payload state weightings
    ...
    def SetCostDyn_ADMM(self):
        ...
        self.p_bar    = 1e-6
        self.h_norm   = 1/(2*self.p_bar)*(norm_2(ql)-1)**2
        ...
        self.sch_norm      = 1/(2*self.p_bar)*(norm_2(scql)-1)**2
        ...
        self.J2_k_admm     = self.J2_k + self.p1/2*xl_const_v.T@xl_const_v + self.p1/2*Wl_const_v.T@Wl_const_v + self.sch_norm
...
class Gradient_Solver:
    def __init__(self, sysm_para, horizon, xl, Wl, scxl, scWl, nv, P_auto, P_pinv, P_ns, e_abs, e_rel):
        ...
        # tracking loss
        self.w_track = 1
        error_pl = self.xl[0:3] - self.xl_ref[0:3]
        error_vl = self.xl[3:6] - self.xl_ref[3:6]
        ql = self.xl[6:10]
        ref_ql = self.xl_ref[6:10]
        Rl            = self.q_2_rotation(ql)
        Rdl           = self.q_2_rotation(ref_ql)
        error_Rl      = Rdl.T@Rl - Rl.T@Rdl
        att_error_l   = 1/2*self.vee_map(error_Rl) # attitude error in Lie group
    ...
    def loss(self,Opt_Sol1,Opt_Sol2,Ref_xl,p1):
        ...
        for k in range(self.N):
            ...
            error_pl = np.reshape(xl_k[0:3,0]-refxl_k[0:3,0],(3,1))
            error_vl = np.reshape(xl_k[3:6,0]-refxl_k[3:6,0],(3,1))
            ql       = xl_k[6:10,0]
            ref_ql   = refxl_k[6:10,0]
            Rl            = self.q_2_rotation(ql)
            Rdl           = self.q_2_rotation(ref_ql)
            error_Rl      = Rdl.T@Rl - Rl.T@Rdl
