...
class MPC:
    ...
    def SetLearnablePara(self):
        ...
        self.beta     = SX.sym('beta',1,1) # tilt angle of the cable
        self.dtension   = SX.sym('dten',1,1) # tension reference change of the payload
        self.n_pi     = self.para_i.numel()
        self.n_pl     = self.para_l.numel()
        self.n_beta   = self.beta.numel()
        self.n_dt     = self.dtension.numel()

    def SetQuadrotorCostDyn(self,index):
        self.ref_xi   = SX.sym('ref_xi',self.n_xi,1)
        self.ref_ui   = SX.sym('ref_ui',self.n_ui,1)
        ref_modifier  = vertcat(self.cl0*sin(self.beta)*cos(index*self.alpha),self.cl0*sin(self.beta)*sin(self.alpha),(cos(self.beta)-1)*self.cl0)
        p_error_i     = self.xi[0:3,0] - (self.ref_xi[0:3,0]+ref_modifier)
        ...
        self.Ji_kfn   = Function('Ji_k',[self.xi, self.ui, self.ref_xi, self.ref_ui, self.para_i, self.beta],[self.Ji_k],['xi0', 'ui0', 'refxi0', 'refui0', 'parai0', 'beta0'],['Ji_kf'])
        # terminal cost
        self.Ji_N     = 1/2 * track_error_i.T@self.Qi_N@track_error_i
        self.Ji_Nfn   = Function('Ji_N',[self.xi, self.ref_xi, self.para_i, self.beta],[self.Ji_N],['xi0', 'refxi0', 'parai0', 'beta0'],['Ji_Nf'])
    ...
    def SetPayloadCostDyn(self):
        ...
        ctrl_error_l  = self.ul - (self.ref_ul + self.dtension*np.ones((self.n_ul,1)))
        ...
        self.Jl_kfn   = Function('Jl_k',[self.xl, self.ul, self.ref_xl, self.ref_ul, self.para_l, self.dtension],[self.Jl_k],['xl0', 'ul0', 'refxl0', 'reful0', 'paral0','dt0'],['Jl_kf'])
        # terminal cost
        self.Jl_N     = 1/2 * track_error_l.T@self.Ql_N@track_error_l
        self.Jl_Nfn   = Function('Jl_N',[self.xl, self.ref_xl, self.para_l, self.dtension],[self.Jl_N],['xl0', 'refxl0', 'paral0', 'dt0'],['Jl_Nf'])
    ...
    def MPCsolverQuadrotor(self, xi_fb, xq_traj, xl_traj, ul_traj, Ref_xi, Ref_ui, Para_i, index, beta):
        assert hasattr(self, 'xi'), "Define the quadrotor's state variable first!"
        assert hasattr(self, 'ui'), "Define the quadrotor's contrl variable first!"
        self.SetQuadrotorCostDyn(index)
        ...
        for k in range(self.N):
            ...
            J       += self.Ji_kfn(xi0=xk,ui0=uk,refxi0=Ref_xi[:,k],refui0=Ref_ui[:,k],parai0=Para_i, beta0=beta)['Ji_kf']   
            if k>0:       
        ...
        J       += self.Ji_Nfn(xi0=xk,refxi0=Ref_xi[:,-1],parai0=Para_i,beta0=beta)['Ji_Nf']
        ...
        dJk_fn   = Function('dJk',[self.xi, self.ui, self.xl, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta],[dJk_aug],['xi0', 'ui0', 'xl0', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'],['dJkf'])
        dJN_fn   = Function('dJN',[self.xi, self.xl, self.xq, self.ref_xi, self.para_i, self.beta],[dJN_aug],['xi0', 'xl0', 'xq0', 'refxi0', 'parai0', 'beta0'],['dJNf'])
        ...
        costate_traj_opt[self.N-1:self.N,:] = np.reshape(dJN_fn(xi0=state_traj_opt[-1,:],xl0=xl_traj[-1,:],xq0=xq,refxi0=Ref_xi[:,-1],parai0=Para_i,beta0=beta)['dJNf'].full(),(1,self.n_xi))
        for k in range(self.N-1, 0, -1): # horizon = N
            ...
            for i in range(self.nq):
                xi_opt      = xq_traj[i]
                xq[:,i:i+1] = np.reshape(xi_opt[k,:],(self.n_xi,1))
            dcdx_k    = np.reshape(dJk_fn(xi0=xi_curr, ui0=ui_curr, xl0=xl_traj[k,:], xq0=xq, refxi0=Ref_xi[:,k], refui0=Ref_ui[:,k], parai0=Para_i, beta0=beta)['dJkf'].full(),(self.n_xi,1))
    ...
    def MPCsolverPayload(self, xl_fb, xq_traj, Ref_xl, Ref_ul, Para_l, Jl, rg, dtension):
        assert hasattr(self, 'xl'), "Define the payload's state variable first!"
        ...
        for k in range(self.N):
            ...
            # integrate the cost function till the end of horizon
            J       += self.Jl_kfn(xl0=xk, ul0=uk, refxl0=Ref_xl[:,k], reful0=Ref_ul[:,k], paral0=Para_l, dt0=dtension)['Jl_kf']
            xq   = np.zeros((self.n_xi,self.nq))
            for i in range(self.nq):
                xi_opt      = xq_traj[i]
                xq[:,i:i+1] = np.reshape(xi_opt[k,:],(self.n_xi,1))
        ...
        # add the final cost
        J     += self.Jl_Nfn(xl0=xk, refxl0=Ref_xl[:,-1], paral0=Para_l, dt0=dtension)['Jl_Nf']
        xq     = np.zeros((self.n_xi,self.nq))
        for i in range(self.nq):
            xi_opt      = xq_traj[i]
            xq[:,i:i+1] = np.reshape(xi_opt[-1,:],(self.n_xi,1))
        ...
        dJlk_fn  = Function('dJlk',[self.xl, self.ul, self.xq, self.ref_xl, self.ref_ul, self.para_l, self.dtension], [dJk], ['xl0', 'ul0', 'xq0', 'refxl0', 'reful0', 'paral0', 'dt0'], ['dJlkf'])
        dJlN_fn  = Function('dJlN',[self.xl, self.xq, self.ref_xl, self.para_l, self.dtension],[dJN],['xl0', 'xq0', 'refxl0', 'paral0', 'dt0'],['dJlNf'])
        ...
        costate_traj_opt[self.N-1:self.N,:] = np.reshape(dJlN_fn(xl0=state_traj_opt[-1,:], xq0=xq, refxl0=Ref_xl[:,-1], paral0=Para_l, dt0=dtension)['dJlNf'].full(),(1,self.n_xl))
        for k in range(self.N-1,0,-1):
            ...
            for i in range(self.nq):
                xi_opt      = xq_traj[i]
                xq[:,i:i+1] = np.reshape(xi_opt[k,:],(self.n_xi,1))
            dcdx_k    = np.reshape(dJlk_fn(xl0=xl_curr, ul0=ul_curr, xq0=xq, refxl0=Ref_xl[:,k], reful0=Ref_ul[:,k], paral0=Para_l, dt0=dtension)['dJlkf'].full(),(self.n_xl,1))
    ...
    def Distributed_forwardMPC(self, xq_fb, xl_fb, xq_traj_prev, uq_traj_prev, xl_traj_prev, ul_traj_prev, Ref_xq, Ref_uq, Ref_xl, Ref_ul, Para_q, Para_l, Jl, rg, beta, dtension):
        ...
        while max_violation>=epsilon and k<=k_max:
            ...
            for i in range(self.nq): # later, we will upgrade these iterations to parallel computing
                ...
                Para_i      = Para_q[i]
                opt_sol_i   = self.MPCsolverQuadrotor(xi_fb, xq_traj, xl_traj, ul_traj, Ref_xi, Ref_ui, Para_i, i, beta)
            ...
            opt_sol_l   = self.MPCsolverPayload(xl_fb, xq_traj, Ref_xl, Ref_ul, Para_l, Jl, rg, dtension)
    ...
    def DiffKKT_quadrotor(self, index):
        ...
        self.E_p        = np.zeros((self.n_xi, self.n_beta)) # used at k=0,...,N-1 for beta = tilt angle (beta)
        ...
        self.ddL0xx_fn  = Function('ddL0xx',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0xx], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0xxf'])
        self.ddL0xu     = jacobian(self.dL0x, self.ui)
        self.ddL0xu_fn  = Function('ddL0xu',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0xu], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0xuf'])
        self.ddL0ux     = jacobian(self.dL0u, self.xi)
        self.ddL0ux_fn  = Function('ddL0ux',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0ux], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0uxf'])
        self.ddL0uu     = jacobian(self.dL0u, self.ui)
        self.ddL0uu_fn  = Function('ddL0uu',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0uu], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0uuf'])
        # when beta = tilt angle (beta), denoted by 'p' in the code
        self.ddL0xp     = jacobian(self.dL0x, self.beta)  
        self.ddL0xp_fn  = Function('ddL0xp',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0xp], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0xpf'])
        self.ddL0up     = jacobian(self.dL0u, self.beta)
        self.ddL0up_fn  = Function('ddL0up',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0up], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0upf'])
        ...
        self.ddL0xxl_fn = Function('ddL0xxl',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0xxl], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0xxlf'])
        self.ddL0uxl    = jacobian(self.dL0u, self.xl)
        self.ddL0uxl_fn = Function('ddL0uxl',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0uxl], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0uxlf'])
        # when beta = ul_0, denoted by 'ul' in the code
        self.ddL0xul    = jacobian(self.dL0x, self.ti)@self.ei
        self.ddL0xul_fn = Function('ddL0xti',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0xul], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0xulf'])
        self.ddL0uul    = jacobian(self.dL0u, self.ti)@self.ei 
        self.ddL0uul_fn = Function('ddL0uti',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddL0uul], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddL0uulf'])
        
        """
        second-order derivative of Lk, k=1,...,N-1
        """
        self.ddLkxx     = jacobian(self.dLkx, self.xi)
        self.ddLkxx_fn  = Function('ddLkxx',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddLkxx], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddLkxxf'])
        self.ddLkxu     = jacobian(self.dLkx, self.ui)
        self.ddLkxu_fn  = Function('ddL0xu',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddLkxu], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddLkxuf'])
        self.ddLkux     = jacobian(self.dLku, self.xi)
        self.ddLkux_fn  = Function('ddL0ux',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddLkux], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddLkuxf'])
        self.ddLkuu     = jacobian(self.dLku, self.ui)
        self.ddLkuu_fn  = Function('ddL0uu',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddLkuu], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddLkuuf'])
        # when beta = tilt angle (beta), denoted by 'p' in the code
        self.ddLkxp     = jacobian(self.dLkx, self.beta)  
        self.ddLkxp_fn  = Function('ddLkxp',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddLkxp], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddLkxpf'])
        self.ddLkup     = jacobian(self.dLku, self.beta)
        self.ddLkup_fn  = Function('ddLkup',[self.xi, self.ui, self.xl, self.ti, self.ind_q, self.next_cs, self.xq, self.ref_xi, self.ref_ui, self.para_i, self.beta], [self.ddLkup], 
                                   ['xi0', 'ui0', 'xl0', 'ti0', 'i0', 'cs_n', 'xq0', 'refxi0', 'refui0', 'parai0', 'beta0'], ['ddLkupf'])
        # when beta = xi_init, denoted by 'xi' in the code
        ...
        self.ddLNxx_fn  = Function('ddLNxx',[self.xi, self.xl, self.xq, self.ref_xi, self.para_i, self.beta], [self.ddLNxx],
                                   ['xi0', 'xl0', 'xq0', 'refxi0', 'parai0', 'beta0'], ['ddLNxxf'])
        # when beta = tilt angle (beta), denoted by 'p' in the code
        self.ddLNxp     = jacobian(self.dLNx, self.beta)
        self.ddLNxp_fn  = Function('ddLNxp',[self.xi, self.xl, self.xq, self.ref_xi, self.para_i, self.beta], [self.ddLNxp],
                                   ['xi0', 'xl0', 'xq0', 'refxi0', 'parai0', 'beta0'], ['ddLNxpf'])
        # when beta = xi_init, denoted by 'xi' in the code
    ...
    def GetAuxSys_quadrotor(self, index, xq_traj, uq_traj, xl_traj, ul_traj, cx_quad_traj, Ref_xq, Ref_uq, CTRL_gain, beta):
        ...
        for k in range(self.N):
            ...
            if k == 0:
                ...
                matLxx      += [self.ddL0xx_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0xxf'].full()]
                matLxu      += [self.ddL0xu_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0xuf'].full()]
                matLux      += [self.ddL0ux_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0uxf'].full()]
                matLuu      += [self.ddL0uu_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0uuf'].full()]
                matLxp      += [self.ddL0xp_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0xpf'].full()]            
                matLup      += [self.ddL0up_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0upf'].full()]
                matLxxi     += [self.ddL0xxi]
                matLuxi     += [self.ddL0uxi]
                matLxxl     += [self.ddL0xxl_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0xxlf'].full()]
                matLuxl     += [self.ddL0uxl_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0uxlf'].full()]
                matLxul     += [self.ddL0xul_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0xulf'].full()]
                matLuul     += [self.ddL0uul_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddL0uulf'].full()]
            else:
                matE_xl     += [self.E_xlk]
                matE_ul     += [self.E_ulk]
                matLxx      += [self.ddLkxx_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,xq0=curr_xq,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddLkxxf'].full()]
                matLxu      += [self.ddLkxu_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,xq0=curr_xq,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddLkxuf'].full()]
                matLux      += [self.ddLkux_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,xq0=curr_xq,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddLkuxf'].full()]
                matLuu      += [self.ddLkuu_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,xq0=curr_xq,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddLkuuf'].full()]
                matLxp      += [self.ddLkxp_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,xq0=curr_xq,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddLkxpf'].full()]
                matLup      += [self.ddLkup_fn(xi0=curr_x,ui0=curr_u,xl0=curr_xl,ti0=curr_ti,i0=index,cs_n=next_cs,xq0=curr_xq,refxi0=curr_refx,refui0=curr_refu,parai0=Para_i,beta0=beta)['ddLkupf'].full()]
        ...
        matLxx     += [self.ddLNxx_fn(xi0=curr_x,xl0=curr_xl,xq0=curr_xq,refxi0=curr_refx,parai0=Para_i,beta0=beta)['ddLNxxf'].full()]
        matLxp     += [self.ddLNxp_fn(xi0=curr_x,xl0=curr_xl,xq0=curr_xq,refxi0=curr_refx,parai0=Para_i,beta0=beta)['ddLNxpf'].full()]
    ...
    def DiffKKT_payload(self, index):
        ...
        self.El_p       = np.zeros((self.n_xl, self.n_dt)) # used at k=0,...,N-1 for beta = tension change dh)
        ...
        self.ddLl0xx_fn = Function('ddLl0xx',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0xx],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0xxf'])
        self.ddLl0xu    = jacobian(self.dLl0x, self.ul)
        self.ddLl0xu_fn = Function('ddLl0xu',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0xu],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0xuf'])
        self.ddLl0ux    = jacobian(self.dLl0u, self.xl)
        self.ddLl0ux_fn = Function('ddLl0ux',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0ux],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0uxf'])
        self.ddLl0uu    = jacobian(self.dLl0u, self.ul)
        self.ddLl0uu_fn = Function('ddLl0uu',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0uu],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0uuf'])
        # when beta = tension change (dh), denoted by 'p' in the code
        self.ddLl0xp    = jacobian(self.dLl0x, self.dtension)
        self.ddLl0xp_fn = Function('ddLl0xp',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0xp],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0xpf'])
        self.ddLl0up    = jacobian(self.dLl0u, self.dtension)
        self.ddLl0up_fn = Function('ddLl0up',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0up],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0upf'])
        # when beta = xl_init, denoted by 'xl' in the code
        ...
        self.ddLl0xxi_fn= Function('ddLl0xxi',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0xxi],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0xxif'])
        self.ddLl0uxi   = jacobian(self.dLl0u, self.xq[:,index])
        self.ddLl0uxi_fn= Function('ddLl0uxi',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0uxi],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0uxif'])
        # when beta = load's inertial parameters, denoted by 'lp' in the code
        self.ddLl0xlp   = jacobian(self.dLl0x, self.loadp)
        self.ddLl0xlp_fn= Function('ddLl0xlp',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0xlp],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0xlpf'])
        self.ddLl0ulp   = jacobian(self.dLl0u, self.loadp)
        self.ddLl0ulp_fn= Function('ddLl0ulp',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLl0ulp],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLl0ulpf'])
        
        """
        second-order derivative of Llk, k=1,...,N-1
        """
        self.ddLlkxx    = jacobian(self.dLlkx, self.xl)
        self.ddLlkxx_fn = Function('ddLlkxx',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkxx],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkxxf'])
        self.ddLlkxu    = jacobian(self.dLlkx, self.ul)
        self.ddLlkxu_fn = Function('ddLlkxu',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkxu],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkxuf'])
        self.ddLlkux    = jacobian(self.dLlku, self.xl)
        self.ddLlkux_fn = Function('ddLlkux',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkux],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkuxf'])
        self.ddLlkuu    = jacobian(self.dLlku, self.ul)
        self.ddLlkuu_fn = Function('ddLlkuu',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkuu],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkuuf'])
        # when beta = tension change (dh), denoted by 'p' in the code
        self.ddLlkxp    = jacobian(self.dLlkx, self.dtension)
        self.ddLlkxp_fn = Function('ddLlkxp',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkxp],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkxpf'])
        self.ddLlkup    = jacobian(self.dLlku, self.dtension)
        self.ddLlkup_fn = Function('ddLlkup',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkup],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkupf'])
        # when beta = xl_init, denoted by 'xl' in the code
        ...
        self.ddLlkxlp_fn= Function('ddLlkxlp',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkxlp],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkxlpf'])
        self.ddLlkulp   = jacobian(self.dLlku, self.loadp)
        self.ddLlkulp_fn= Function('ddLlkulp',[self.xl, self.ul, self.xq, self.next_csl, self.ref_xl, self.ref_ul, self.loadp, self.para_l, self.dtension], [self.ddLlkulp],
                                   ['xl0', 'ul0', 'xq0', 'csl_n', 'refxl0', 'reful0', 'loadp0', 'paral0', 'dt0'], ['ddLlkulpf'])
        """
        second-order derivative of LlN
        """
        self.ddLlNxx    = jacobian(self.dLlNx, self.xl)
        self.ddLlNxx_fn = Function('ddLlNxx',[self.xl, self.xq, self.ref_xl, self.para_l, self.dtension], [self.ddLlNxx], ['xl0', 'xq0', 'refxl0', 'paral0', 'dt0'], ['ddLlNxxf'])
        # when beta = tension change (dh), denoted by 'p' in the code
        self.ddLlNxp    = jacobian(self.dLlNx, self.dtension)
        self.ddLlNxp_fn = Function('ddLlNxp',[self.xl, self.xq, self.ref_xl, self.para_l, self.dtension], [self.ddLlNxp], ['xl0', 'xq0', 'refxl0', 'paral0', 'dt0'], ['ddLlNxpf'])
        # when beta = xl_init, denoted by 'xl' in the code
    ...
    def GetAuxSys_payload(self, index, xl_traj, ul_traj, xq_traj, cx_load_traj, Ref_xl, Ref_ul, Para_l, loadp, dtension):
        ...
        for k in range(self.N):
            ...
            if k==0:
                ...
                matLlxx    += [self.ddLl0xx_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0xxf'].full()]
                matLlxu    += [self.ddLl0xu_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0xuf'].full()]
                matLlux    += [self.ddLl0ux_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0uxf'].full()]
                matLluu    += [self.ddLl0uu_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0uuf'].full()]
                matLlxp    += [self.ddLl0xp_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0xpf'].full()]
                matLlup    += [self.ddLl0up_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0upf'].full()]
                matLlxxl   += [self.ddLl0xxl]
                matLluxl   += [self.ddLl0uxl]
                matLlxxi   += [self.ddLl0xxi_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0xxif'].full()]
                matLluxi   += [self.ddLl0uxi_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0uxif'].full()]
                matLlxlp   += [self.ddLl0xlp_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0xlpf'].full()]
                matLlulp   += [self.ddLl0ulp_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLl0ulpf'].full()]
            else:
                matEl_xi   += [self.El_xik]
                matLlxx    += [self.ddLlkxx_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkxxf'].full()]
                matLlxu    += [self.ddLlkxu_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkxuf'].full()]
                matLlux    += [self.ddLlkux_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkuxf'].full()]
                matLluu    += [self.ddLlkuu_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkuuf'].full()]
                matLlxp    += [self.ddLlkxp_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkxpf'].full()]
                matLlup    += [self.ddLlkup_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkupf'].full()]
                matLlxxl   += [self.ddLlkxxl]
                matLluxl   += [self.ddLlkuxl]
                matLlxxi   += [self.ddLlkxxi]
                matLluxi   += [self.ddLlkuxi]
                matLlxlp   += [self.ddLlkxlp_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkxlpf'].full()]
                matLlulp   += [self.ddLlkulp_fn(xl0=curr_xl,ul0=curr_ul,xq0=curr_xq,csl_n=next_csl,refxl0=curr_refxl,reful0=curr_reful,loadp0=loadp,paral0=Para_l,dt0=dtension)['ddLlkulpf'].full()]
        curr_xl    = xl_traj[-1,:]
        curr_xq    = np.zeros((self.n_xi,self.nq))
        for i in range(self.nq):
            xi_opt      = xq_traj[i]
            curr_xq[:,i:i+1] = np.reshape(xi_opt[-1,:],(self.n_xi,1))
        curr_refxl = Ref_xl[:,-1]
        matLlxx     += [self.ddLlNxx_fn(xl0=curr_xl,xq0=curr_xq,refxl0=curr_refxl,paral0=Para_l,dt0=dtension)['ddLlNxxf'].full()]
        matLlxp     += [self.ddLlNxp_fn(xl0=curr_xl,xq0=curr_xq,refxl0=curr_refxl,paral0=Para_l,dt0=dtension)['ddLlNxpf'].full()]
...
class MPC_gradient: # compute the gradients of the MPC's solution trajectories w.r.t the interested hyperparameters (using the PDP method)
    def __init__(self, xi, xl, ul, beta, dtension, loadp, horizon):
        self.n_xi = xi.numel()
        self.n_xl = xl.numel()
        self.n_ul = ul.numel()
        self.n_beta = beta.numel()
        self.n_dt = dtension.numel()
    ...
    def Gradient_solver_quadrotor(self,auxSys):
        ...
        W_p      = self.N * [np.zeros((self.n_xi, self.n_beta))] # for beta = tilt angle (denoted as 'p')
        ...
        X_p_0     = np.zeros((self.n_xi, self.n_beta))
    ...
    def Gradient_solver_payload_2_quadi(self,auxSysl): # relative to the ith quadrotor
        ...
        Wl_p      = self.N * [np.zeros((self.n_xl, self.n_dt))] # for beta = tension change (denoted as 'p')
        ...
        # when beta = tension change (denoted as 'p')
        Xl_p_0     = np.zeros((self.n_xl, self.n_dt))
...
class Sensitivity_propagation: # compute the sensitivities (gradients) of the actual system states w.r.t the interested hyperparameters (online training)
    def __init__(self, uav_para, xi, xl, ul, beta, dtension, loadp, horizon_loss, horizon):
        self.nq      = int(uav_para[4])
        self.n_xi    = xi.numel()
        self.n_xl    = xl.numel()
        self.n_ul    = ul.numel()
        ...
        self.xi      = xi
        self.xl      = xl
        self.zi      = self.xi[2,0] # zi: height of the i-th quadrotor
        self.zl      = self.xl[2,0] # zl: height of the load
        ...
        self.obsh1   = SX.sym('zobs1',1,1) # height of the bottom of the passage
        self.obsh2   = SX.sym('zobs2',1,1) # height of the top of the passage
        self.dist    = SX.sym('do',1,1) # minimial distance between the quadrotors and the obstacle
        weight_hl_i  = 1e2 # weight for the high-level loss function for the i-th quadrotor, this should be very small compared to weight_h_l
        weight_hl_tck  = 1e5 # weight for the high-level loss function for the load
        weight_hl_obs  = 1e5
        gamma0       = 1.25 # 2
        gamma1       = 2
        gamma2       = 10
        alpha_loss   = exp(-gamma0*self.dist)
        loss_track_i = weight_hl_i * (self.zi - self.refxi[2,0])**2
        loss_obs_i   = weight_hl_i * exp(-gamma1*(self.obsh1 - self.zi))
        self.loss_i  = (1-alpha_loss) * loss_track_i + alpha_loss * loss_obs_i
        loss_track_l = weight_hl_tck * (self.zl - self.refxl[2,0])**2
        loss_obs_l   = weight_hl_obs * exp(-gamma2*(self.zl - self.obsh2))
        self.loss_l  = (1-alpha_loss) *loss_track_l + alpha_loss * loss_obs_l
    ...
    def quadrotor_sensitivity(self, Quads_Mat, Load_Mat, Xl_pl, sumXi_pl, index):
        ...
        Xi_pi       = (self.N_loss+1)*[np.zeros((self.n_xi, self.n_beta))] # X^i_pi: the gradient of the i-th quadrotor's state w.r.t its tilt angle
        Xi_pl       = (self.N_loss+1)*[np.zeros((self.n_xi, self.n_dt))] # X^i_pl: the gradient of the i-th quadrotor's state w.r.t the payload height change
        Xl_pi       = (self.N_loss+1)*[np.zeros((self.n_xl, self.n_beta))] # X^l_pi: the gradient of the payload's state w.r.t the i-th quadrotor's tilt angle
    ...
    def quadrotor_sensitivity_parallel(self, Quads_Mat, Load_Mat, Xl_pl, index):
        ...
        Xi_pi       = (self.N_loss+1)*[np.zeros((self.n_xi, self.n_beta))] # X^i_pi: the gradient of the i-th quadrotor's state w.r.t its tilt angle
        Xi_pl       = (self.N_loss+1)*[np.zeros((self.n_xi, self.n_dt))] # X^i_pl: the gradient of the i-th quadrotor's state w.r.t the payload height change
        Xl_pi       = (self.N_loss+1)*[np.zeros((self.n_xl, self.n_beta))] # X^l_pi: the gradient of the payload's state w.r.t the i-th quadrotor tilt angle
        FXi_pl      = (self.N_loss)*[np.zeros((self.n_xi, self.n_beta))] # list for saving the product dfldxi@Xi_pl[t]
    ...
    def payload_sensitivity(self, Load_Mat, sumXi_pl):
        ...
        Xl_pl       = (self.N_loss+1)*[np.zeros((self.n_xl, self.n_dt))] # X^l_pl: the gradient of the payload's state w.r.t its height change
    ...
    def Distributed_sensitivity(self, Quads_Mat, Load_Mat):
        ...
        Xl_Pi_Prev    = []
        for i in range(self.nq):
            Xi_pi_prev    = (self.N_loss+1)*[np.zeros((self.n_xi, self.n_beta))]
            Xi_pl_prev    = (self.N_loss+1)*[np.zeros((self.n_xi, self.n_dt))]
            Xl_pi_prev    = (self.N_loss+1)*[np.zeros((self.n_xl, self.n_beta))]
            Xi_Pi_Prev   += [Xi_pi_prev]
            Xi_Pl_Prev   += [Xi_pl_prev]
            Xl_Pi_Prev   += [Xl_pi_prev]
        
        Xl_pl         = (self.N_loss+1)*[np.zeros((self.n_xl, self.n_dt))] 
        Xl_pl_prev    = (self.N_loss+1)*[np.zeros((self.n_xl, self.n_dt))]


        while max_violation>=epsilon and k<=k_max:
            sumXi_pl         = (self.N_loss)*[np.zeros((self.n_xl, self.n_dt))] # initialize sumXi_pl
            
            viol_X_list      = []
            All_quad_sens_Xipi    = [] # list for saving all the updated sensitivity trajectories during the 'for' loop
            All_quad_sens_Xipl    = []
            All_quad_sens_Xlpi    = []
            for i in range(self.nq): # later, we will upgrade these iterations to parallel computing
                ...
                All_quad_sens_Xipi += [Xi_pi]
                All_quad_sens_Xipl += [Xi_pl]
                All_quad_sens_Xlpi += [Xl_pi]
                sum_Xi_pi_viol = 0
                sum_Xi_pl_viol = 0
                sum_Xl_pi_viol = 0
        ...
        return All_quad_sens_Xipi, All_quad_sens_Xipl, All_quad_sens_Xlpi, Xl_pl
    ...
    def loss_quadrotor(self, xi, refxi, distance, obs1):
        # define the quadrotor's high-level loss function
        lossi_fn  = Function('lossi',[self.xi, self.refxi, self.dist, self.obsh1], [self.loss_i], ['xi0', 'refxi0', 'dist0', 'obsh10'], ['lossif'])
        loss_quad_i = lossi_fn(xi0=xi,refxi0=refxi,dist0=distance,obsh10=obs1)['lossif'].full()
        return loss_quad_i
    
    def loss_load(self, xl, refxl, distance ,obs2):
        # define the load's high-level loss function
        lossl_fn  = Function('lossl',[self.xl, self.refxl, self.dist, self.obsh2], [self.loss_l], ['xl0', 'refxl0','dist0', 'obsh20'], ['losslf'])
        loss_load = lossl_fn(xl0=xl,refxl0=refxl,dist0=distance,obsh20=obs2)['losslf'].full()
        return loss_load
    ...
    def ChainRule_quadrotor_i(self, index, Quad_State, Ref_Quad, All_quad_sens_Xipl, distance, obs1):
        # define the gradient of loss_i w.r.t the state xi
        dlids     = jacobian(self.loss_i, self.xi)
        dlids_fn  = Function('dlids',[self.xi, self.refxi, self.dist, self.obsh1], [dlids], ['xi0','refxi0','dist0','obsh10'], ['dlidsf'])
        # initialize the parameter gradient
        dpil      = np.zeros((1,self.n_dt))
        # initialize the high-level loss_i
        loss_i    = 0
        # select the corresponding sensitivity trajectroy from the list 'All_quad_sens'
        Xi_pl     = All_quad_sens_Xipl[index] # for the i-th quadrotor
        for t in range(self.N_loss):
            xi         = Quad_State[len(Quad_State)-self.N_loss+t][index]
            refxi      = Ref_Quad[len(Ref_Quad)-self.N_loss+t][index]
            loss_i_t   = self.loss_quadrotor(xi,refxi, distance, obs1)
            loss_i    += loss_i_t
            Dlids      = dlids_fn(xi0=xi,refxi0=refxi,dist0=distance,obsh10=obs1)['dlidsf'].full()
            Xi_pl_t1   = Xi_pl[t+1] # we skip the initial state at t=0, as its gradient w.r.t the weightings is zero!
            dpil       += Dlids@Xi_pl_t1
        return dpil, loss_i
    ...
    def ChainRule_load(self, Load_State, Ref_Load, Xl_pl, Dist_s ,obs2):
        # define the gradient of loss_l w.r.t the state xl
        dllds     = jacobian(self.loss_l, self.xl)
        dllds_fn  = Function('dllds',[self.xl, self.refxl, self.dist, self.obsh2], [dllds], ['xl0','refxl0','dist0', 'obsh20'], ['dlldsf'])
        # initialize the parameter gradient
        dpl       = np.zeros((1,self.n_dt))
        # initialize the high-level loss_l
        loss_l    = 0
        for t in range(self.N_loss):
            xl         = Load_State[len(Load_State)-self.N_loss+t] 
            refxl      = Ref_Load[len(Ref_Load)-self.N_loss+t]
            distance   = Dist_s[len(Dist_s)-self.N_loss+t]
            loss_l_t   = self.loss_load(xl,refxl,distance,obs2)
            loss_l    += loss_l_t
            Dllds      = dllds_fn(xl0=xl,refxl0=refxl,dist0=distance,obsh20=obs2)['dlldsf'].full()
