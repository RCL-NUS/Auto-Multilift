...
Df_in, Df_h, Df_out = 4, 10, 1 # for learning the configuration
npi        = 2*nwsi + nui
npl        = 2*nwsl + nul
nlp        = len(loadp)
    ...
    while delta_loss>=eps:
        ...
        for k in range(N): 
            ...
            if (k%ratio)==0: 
                ...
                dist = pl[0,0] # only x component is used
                Dist_s += [dist]
                gamma0 = 0.5 #0.5
                angle_t = angle_min + (ta_max - angle_min) * np.exp(-gamma0*dist**2)
                Angles_t += [angle_t]
                dangle_t   = -2*gamma0*dist*(ta_max - angle_min) * np.exp(-gamma0*dist**2)*vl[0,0]
                ...
                Ref_Quad_s  += [Ref0d_xq]
                Ref_Load_s  += [Ref0d_l]
                nn_g_input = np.vstack((angle_t, dangle_t, Ref0d_l[2,0]-pl[2,0], Ref0d_l[5,0]-vl[2,0])) # previously 3, angle_t, z-zref, vz-vzref
                ...
                if trainmode == 'c':
                    ...
                    Load_k_Mat    = []
                    for i in range(nq):
                        auxSys_i    = DistMPC.GetAuxSys_quadrotor(i,xq_traj,uq_traj,xl_traj,ul_traj,cx_quad_traj,Ref_xq,Ref_uq,CTRL_gain, angle_t)
                ...
                if flag_train == 1:
                    ...
                    if trainmode == 'c':
                        start_time = TM.time()
                        if sensmode == 's':
                            All_quad_sens_Xipi, All_quad_sens_Xipl, All_quad_sens_Xlpi, Xl_pl = Sensprop.Distributed_sensitivity(Quads_Mat_s,Load_Mat_s)
                        else:
                    ...
                    for i in range(nq):
                        if trainmode == 'c':
                            dlidt, loss_i = Sensprop.ChainRule_quadrotor_i(i,Quad_State_s,Ref_Quad_s,All_quad_sens_Xipl, dist, obs1)
                        else:
                    ...
                    if trainmode == 'c':
                        ...
                        dlldh, loss_l = Sensprop.ChainRule_load_openloop(Load_State_s,Ref_Load_s,xl_traj, Ref_xl, Xl_pl)
                    system_loss  += loss_l
                    # dldt         += dlldh
                    dlldp         = dlldh@confg_i_grad
                    norm_grad     = LA.norm(dlldp)
                    print('k_train=',k_train,'time step=',k,'loss_l=',loss_l,'norm_grad=',format(norm_grad,'.4f'),'sensmode:',sensmode)
