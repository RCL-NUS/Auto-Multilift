...
class MPC:
    ...
    def MPCsolverPayloadInit_acados(self,gazebo_sim=False):
        ...
        ocpl.dims.np = self.n_xl + self.n_ul + self.n_pl + self.nq*self.n_xi + self.n_lp + 1
        ocpl.parameter_values = np.zeros(self.n_xl + self.n_ul + self.n_pl + self.nq*self.n_xi + self.n_lp + 1)
        ...
        dten   = ocpl.model.p[(self.n_xl+self.n_ul+self.n_pl+self.nq*self.n_xi+self.n_lp):(self.n_xl+self.n_ul+self.n_pl+self.nq*self.n_xi+self.n_lp+1)]
        ...
        ocpl.model.cost_expr_ext_cost = self.Jl_kfn(xl0=ocpl.model.x, ul0=ocpl.model.u, refxl0=Ref_xl, reful0=Ref_ul, paral0=Para_l, dt0=dten)['Jl_kf']\
              + self.hl_kfn(xl0=ocpl.model.x, xq0=Xq)['hlkf']
        ocpl.model.cost_expr_ext_cost_e = self.Jl_Nfn(xl0=ocpl.model.x, refxl0=Ref_xl, paral0=Para_l, dt0=dten)['Jl_Nf']\
              + self.hl_kfn(xl0=ocpl.model.x, xq0=Xq)['hlkf']
        ...
        self.dJlk_fn  = Function('dJlk',[self.xl, self.ul, self.xq, self.ref_xl, self.ref_ul, self.para_l, self.dtension], [dclk_aug], ['xl0', 'ul0', 'xq0', 'refxl0', 'reful0', 'paral0', 'dt0'], ['dJlkf'])
        self.dJlN_fn  = Function('dJlN',[self.xl, self.xq, self.ref_xl, self.para_l, self.dtension],[dclN_aug],['xl0', 'xq0', 'refxl0', 'paral0', 'dt0'],['dJlNf'])
        self.Fcl_fn   = Function('Fl',[self.xl, self.ul, self.xq, self.Jldiag, self.rg], [dModell], ['xl0', 'ul0', 'xq0', 'Jl0', 'rg0'], ['Flf'])



    def MPCsolverPayload_acados(self, xl_fb, xq_traj, Ref_xl, Ref_ul, Para_l, Jl, rg, dtension):
        """
        This function solves the optimal control problem (ocp) using ACADOS
        """
        ...
        for k in range(self.N):
            ...
            self.acados_solver_ql.set(k,'p',np.concatenate((ref_xl,ref_ul,Para_l,xqi,Jl,rg, dtension)))
        ...
        self.acados_solver_ql.set(self.N,'p',np.concatenate((ref_xlN,ref_ul,Para_l,xqiN,Jl,rg, dtension)))
        ...
        costatel_traj_opt[self.N-1:self.N,:] = np.reshape(self.dJlN_fn(xl0=statel_traj_opt[-1,:], xq0=np.transpose(np.reshape(xqiN,(self.nq,self.n_xi))), refxl0=ref_xlN, paral0=Para_l, dt0=dtension)['dJlNf'].full(),(1,self.n_xl))
        for k in range(self.N-1,0,-1):
            ...
            for i in range(self.nq):
                xi = xq_traj[(i*self.n_xi*(self.N+1)+self.n_xi*k):(i*self.n_xi*(self.N+1)+self.n_xi*(k+1))]
                xqi[self.n_xi*i:self.n_xi*(i+1)] = xi
            dcdx_k    = np.reshape(self.dJlk_fn(xl0=xl_curr, ul0=ul_curr, xq0=np.transpose(np.reshape(xqi,(self.nq,self.n_xi))), refxl0=ref_xl, reful0=ref_ul, paral0=Para_l, dt0=dtension)['dJlkf'].full(),(self.n_xl,1))
    ...
    def Distributed_forwardMPC(self, xq_fb, xl_fb, xq_traj_prev, uq_traj_prev, xl_traj_prev, ul_traj_prev, Ref_xq, Ref_uq, Ref_xl, Ref_ul, Para_q, Para_l, Jl, rg, dtension):
...
class MPC_gradient: # compute the gradients of the MPC's solution trajectories w.r.t the interested hyperparameters (using the PDP method)
    def __init__(self, xi, xl, ul, dtension, loadp, horizon):
        self.n_xi = xi.numel()
        self.n_xl = xl.numel()
        self.n_ul = ul.numel()
...
class Sensitivity_propagation: # compute the sensitivities (gradients) of the actual system states w.r.t the interested hyperparameters (online training)
    def __init__(self, uav_para, xi, xl, ul, dtension, loadp, horizon_loss, horizon, gamma0):
        self.nq      = int(uav_para[4])
        self.n_xi    = xi.numel()
        self.n_xl    = xl.numel()
        self.n_ul    = ul.numel()
        ...
        error_Rl     = Rdl.T@Rl - Rl.T@Rdl
        att_error_l  = 1/2*self.vee_map(error_Rl)
        self.obsh1   = SX.sym('zobs1',1,1) # height of the top of the passage
        self.obsh2   = SX.sym('zobs2',1,1) # height of the bottom of the passage
        self.dist    = SX.sym('do',1,1) # minimial distance between the quadrotors and the obstacle
        weight_hl_i  = 1e1 # weight for the high-level loss function for the i-th quadrotor, this should be very small compared to weight_h_l
        weight_hl_tck  = 2e3 # weight for the high-level loss function for the load
        weight_hl_obs  = 1e4 #1e5
        weight_hl_atl  = np.array([1e1,1e1,1e1])
        gamma1       = 10
        gamma2       = 25 # 10
        gamma3       = 20
        alpha_loss   = exp(-gamma0*self.dist**4)
        ...
        loss_track_l = weight_hl_tck * (self.zl - self.refxl[2,0])**2 #+ att_error_l.T@np.diag(weight_hl_atl)@att_error_l
        loss_obs_l   = weight_hl_obs * exp(-gamma2*(self.zl - self.obsh2)) 
        # loss_obs2_l  = 0.2*weight_hl_obs * exp(-gamma3*(self.refxl[2,0]+0.15-self.zl)) # 0.25
    ...
    def Distributed_sensitivity(self, Quads_Mat, Load_Mat):
        ...
        while max_violation>=epsilon and k<=k_max:
            ...
            All_quad_sens_Xlpi    = []
            for i in range(self.nq): # later, we will upgrade these iterations to parallel computing
                Xi_pl, sumXi_pl = self.quadrotor_sensitivity(Quads_Mat, Load_Mat, Xl_pl, sumXi_pl,i) 
                ...
                print('sensitivity: iteration:',k,'quadrotor:',i,'viol_Xi_pl=',format(viol_Xi_pl,'.5f'))
                # update
                Xi_Pl_Prev[i] = Xi_pl
        ...
        return  All_quad_sens_Xipl, Xl_pl
    ...
    def ChainRule_quadrotor_i(self, index, Quad_State, Ref_Quad, All_quad_sens_Xipl, Dist_s, obs1):
